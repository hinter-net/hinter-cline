...
const { getPeerPath, getPeerAliases, addPeer, managePeer, getPeerConfig, updatePeerConfig } = require('../src/peer');
const { question, isValidSlug, isValidPublicKey, selectFromList } = require('../src/utils');
...
describe('peer', () => {
    ...
    describe('getPeerConfig', () => {
        it('should read and parse the peer config file', async () => {
            ...
            const expectedPath = path.join(getPeerPath(DATA_PATH, peerAlias), 'hinter.config.json');
    ...
    describe('updatePeerConfig', () => {
        it('should write the peer config file', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(expectedPath, JSON.stringify(config, null, 2));
        });
    });
    ...
    describe('addPeer', () => {
        it('should add a new peer', async () => {
            ...
            question.mockResolvedValueOnce(newPeerAlias).mockResolvedValueOnce(publicKey);
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(expectedConfigPath, JSON.stringify(expectedConfig, null, 2));
        });
        ...
        it('should not add a peer if alias already exists', async () => {
            ...
            isValidSlug.mockReturnValue(true);
            fs.readdir.mockResolvedValue([{ name: 'existing-peer', isDirectory: () => true }]);
            await addPeer(DATA_PATH);
            expect(fs.mkdir).not.toHaveBeenCalled();
        });

        it('should not add a peer with an invalid public key', async () => {
            question.mockResolvedValueOnce('new-peer').mockResolvedValueOnce('invalid-key');
            isValidSlug.mockReturnValue(true);
        ...
        it('should not add a peer if public key already exists', async () => {
            question.mockResolvedValueOnce('new-peer').mockResolvedValueOnce('a'.repeat(64));
            isValidSlug.mockReturnValue(true);
            isValidPublicKey.mockReturnValue(true);
            fs.readdir.mockResolvedValue([{ name: 'existing-peer', isDirectory: () => true }]);
    ...
    describe('managePeer', () => {
        ...
        it('should not rename a peer with an invalid alias', async () => {
            ...
            question.mockResolvedValueOnce('1').mockResolvedValueOnce('invalid alias');
            isValidSlug.mockReturnValue(false);
            await managePeer(DATA_PATH);
        ...
        it('should not rename a peer if alias already exists', async () => {
            fs.readdir.mockResolvedValue([{ name: 'peer1', isDirectory: () => true }, { name: 'existing-peer', isDirectory: () => true }]);
            selectFromList.mockResolvedValue(['peer1']);
            question.mockResolvedValueOnce('1').mockResolvedValueOnce('existing-peer');
            isValidSlug.mockReturnValue(true);
            await managePeer(DATA_PATH);
            expect(fs.rename).not.toHaveBeenCalled();
        });
        ...
        it('should not update a peer if public key already exists', async () => {
            fs.readdir.mockResolvedValue([{ name: 'peer1', isDirectory: () => true }, { name: 'peer2', isDirectory: () => true }]);
            selectFromList.mockResolvedValue(['peer1']);
            question.mockResolvedValueOnce('2').mockResolvedValueOnce('b'.repeat(64));
            isValidPublicKey.mockReturnValue(true);
            fs.readFile.mockResolvedValueOnce(JSON.stringify({ publicKey: 'b'.repeat(64) })); // peer2 config
            fs.readFile.mockResolvedValueOnce(JSON.stringify({ publicKey: 'a'.repeat(64) })); // peer1 config
            await managePeer(DATA_PATH);
            expect(fs.writeFile).not.toHaveBeenCalled();
        });
