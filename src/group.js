...
async function manageGroup(dataPath) {
  ...
  try {
    selectedPeersToRemove = await selectFromList(
      managedGroupPeerAliases,
      "Select peers to remove from group.",
    );
  } catch (e) {
    console.log(e.message);
    return;
  ...
  for (const peerAlias of selectedPeersToRemove) {
    const config = await getPeerConfig(dataPath, peerAlias);
    config["hinter-cline"].groups = config["hinter-cline"].groups.filter(
      (g) => g !== managedGroupAlias,
    );
    await updatePeerConfig(dataPath, peerAlias, config);
  ...
  const peerAliases = await getPeerAliases(dataPath);
  const managedGroupNonmemberPeerAliases = peerAliases.filter(
    (p) => !managedGroupPeerAliases.includes(p),
  );
  if (managedGroupNonmemberPeerAliases.length > 0) {
    let selectedPeersToAdd;
    try {
      selectedPeersToAdd = await selectFromList(
        managedGroupNonmemberPeerAliases,
        "Select peers to add to group.",
      );
    } catch (e) {
      console.log(e.message);
      return;
